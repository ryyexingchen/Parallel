...
float* Gaussian_Elimination_parallel_aligned() {
	//消去过程
	for (int k = 0; k < N; k++) {//归一化
        __m128 vt = _mm_set1_ps(A_align[k][k]);
        int j = k+1;
        while ((long long)(&A_align[k][j])%16){//检查每一行最前面的元素是否对齐，手动处理未对齐的元素
            A_align[k][j] = A_align[k][j] / A_align[k][k];
            j++;
        }
        for ( ; j + 4 <= N; j += 4) {
            __m128 va = _mm_load_ps(A_align[k]+j);
            va = _mm_div_ps(va, vt);
            _mm_store_ps(A_align[k]+j, va);
        }
        for (; j < N; j++) {//处理剩下的元素
            A_align[k][j] = A_align[k][j] / A_align[k][k];
        }
        ...
        A_align[k][k] = 1.0;
        for (int i = k + 1; i < N; i++) {//消元
            __m128 vaik = _mm_set1_ps(A_align[i][k]);
            j = k + 1;
            while ((long long)(&A_align[k][j])%16){//检查每一行最前面的元素是否对齐，手动处理未对齐的元素
            ...
            for ( ; j + 4 <= N; j += 4) {
                __m128 vakj = _mm_load_ps(A_align[k]+j);
                __m128 vaij = _mm_loadu_ps(A_align[i]+j);
                __m128 vx = _mm_mul_ps(vaik, vakj);
                vaij = _mm_sub_ps(vaij, vx);
                _mm_storeu_ps(A_align[i]+j, vaij);
            }
            for (; j < N; j++) {//处理剩下的元素
                A_align[i][j] -= A_align[i][k] * A_align[k][j];
            }
