...
using namespace std;
const int maxN = 30000;//消元子和消元行的个数之和
...
inline void setBit(int& a, int pos, int flag)
{
	if (flag)
		a |= (1 << pos);
	else
	...
	if(myid==0)
		for (int i = 0; i < sumEtemp; i++)
		{
			while ((E[i][0] || Epos[i] > 31) && sumE > 0)
			{
				...
				{
					Rpos[Epos[i]] = sumR;
					memcpy(R[sumR], E[i], sizeof(R[sumR]));//升级为消元子
					sumR++; sumE--;
					break;
				}
			}
